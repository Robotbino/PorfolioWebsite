...
I needed a concept to justify the cosmic theme and the multipage nav, and to lift Creativity above
the generic-starfield baseline that scores poorly on Awwwards.

## Decision

The cosmos *is* the navigation system. Each route is a *Destination* with a **bespoke**
constellation; all constellations share the same star count so they morph 1:1, and the page
transition (*Travel*) pans + morphs between them. The map is **symbolic** (per-page constellations +
