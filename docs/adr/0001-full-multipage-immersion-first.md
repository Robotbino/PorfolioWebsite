...
The portfolio was a single scrolling page with anchor links. The goal was an Awwwards-level
...
Every top-level nav section becomes its own route — `/`, `/work`, `/about`,
`/contact` — and a transition plays on each navigation. I optimise for an engaged *Visitor*:
...
- Trades recruiter scan-speed for immersion. I deliberately do **not** mitigate this with a
